...
modified: 2026-01-15T22:54:29.295+01:00
cssclasses: ""
---
...
Obvious first solution would be just iterate over each element. That would generate a solution of O (s x p). Let's just implement it before diving into optimization.

```python
def successful_pairs_spells_and_potions(spells, potions, success):
	output = []
	for spell in spells:
		ans = 0
...
potions = [1,2,3,4,5]
success = 7
print(successful_pairs_spells_and_potions(spells, potions, success))
...
That was easy. But now we have to optimize it. To be able to optimize it we need to be able to select proper potions from the potions array. For that purpose sorting the array and then searching the min potion with binary search would be an option. By getting the index of the min successful potion we can tell how many of the potion and spell combo would succeed.
